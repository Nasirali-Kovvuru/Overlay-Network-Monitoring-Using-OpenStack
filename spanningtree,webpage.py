...
def get():
    ...
    x.execute('''select * from macs where notify = 1''')
    conn.commit()
    r = [dict((x.description[i][0], value)
              for i, value in enumerate(row)) for row in x.fetchall()]

    return jsonify({'myCollection' : r})
@app.route('/up/<int:post_id>',methods=['POST'])
def ge(post_id):

    x.execute("UPDATE macs SET notify = %s WHERE notify = %s",
        (post_id,1))
    conn.commit()
    conn.close()
